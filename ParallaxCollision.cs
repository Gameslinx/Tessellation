    ...
    public class ParallaxPhysics : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (started == false || dampingFrames <= 150)
            {
                ...
                if (dampingFrames == 150)
                {
                    ScreenMessages.PostScreenMessage("[Parallax Collisions] Done!", 2f, ScreenMessageStyle.UPPER_LEFT);
                }
                return;
            }
        ...
        public float SampleBiplanarTextureCPU(Texture2D tex, Vector2 _ST, Vector3 nextPoint)
        {
            ...
            Vector3 floatUV = Vector3.zero;// Position.floatUV; //SurfaceTextureUVs
            ...
            int UVx = (int)(samplePoint[(int)ma.y] * (tex.width / 1) * _ST.x - floatUV[(int)ma.y]);
            int UVy = (int)(samplePoint[(int)ma.z] * (tex.height / 1) * _ST.y - floatUV[(int)ma.z]);
            int UVMex = (int)(samplePoint[(int)me.y] * (tex.width / 1) * _ST.x - floatUV[(int)me.y]);
            int UVMey = (int)(samplePoint[(int)me.z] * (tex.width / 1) * _ST.y - floatUV[(int)me.z]);
            Color x = Color.black;
            Color y = Color.black;
            if (tex.isReadable)
            {
